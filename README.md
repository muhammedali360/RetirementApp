...
9. **Career years** — shown only when Social Security work history is a constraint (see section 6)
...
In the **Coast** tab, the **Coast number** is the smallest portfolio you'd need *today* so that, left **untouched — no further contributions and no withdrawals** — it still grows enough to clear the ≥90% success threshold at your target retirement age. This is the "Coast FIRE" milestone: the point where your existing balance can coast to retirement on compounding alone.
...
Several visuals make the milestone concrete:

- A **progress bar** showing how far today's balance has come toward the coast number — filled and green once you've cleared it.
- A **stat row** translating the milestone into a decision across three axes: your **coast age** — the age your balance clears the success threshold (labelled with that threshold, e.g. "90% odds") so you could stop contributing — the **saving years left** until then, and the **odds if you stop now** — the success probability of today's balance with no further contributions. The coast number is the balance that lifts those odds to the threshold, so the cards read together as "you're at X% today and reach the threshold at the coast age."
...
- A **compounding split bar** breaking the future nest egg into the coast number you'd hold today versus the growth it earns untouched — most of the finish line is compounding you haven't earned yet.
