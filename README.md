...
- A **stat row** translating the milestone into a decision across three axes: your **coast age** (when saving becomes optional), the **saving years left** until then, and the **odds if you stop now** — the success probability of today's balance with no further contributions. The coast number is the balance that lifts those odds to the success threshold, so the two read together as "you're at X% of the bar you'd need to coast."
